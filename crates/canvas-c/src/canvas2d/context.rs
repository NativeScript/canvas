...
use crate::canvas2d::enums::CanvasRepetition;
use crate::canvas2d::image_data::ImageData;
use crate::canvas2d::matrix::Matrix;
use crate::canvas2d::paint::{PaintStyle, PaintStyleType};
use crate::canvas2d::path::Path;
use crate::canvas2d::text_base_line::TextBaseLine;
use crate::canvas2d::text_metrics::TextMetrics;
use crate::image_asset::ImageAsset;
use crate::webgl::gl::{GLContext, WebGLState};
...
pub fn resize_vulkan(context: &mut CanvasRenderingContext2D, width: f32, height: f32) {
    let alpha = context.alpha;
    let context = &mut context.context;
    Context::resize_vulkan(context, width, height, 0, alpha)
}
...
impl CanvasRenderingContext2D {
    pub fn new(context: Context, gl_context: canvas_core::gl::GLContext, alpha: bool) -> Self {
        Self {
            context,
            gl_context,
            alpha,
...
#[cfg(any(target_os = "android", target_os = "ios"))]
#[no_mangle]
pub extern "C" fn canvas_native_raf_reference(value: *const crate::Raf) {
    if value.is_null() {
        return;
    }
...
#[cfg(any(target_os = "android", target_os = "ios"))]
#[no_mangle]
pub extern "C" fn canvas_native_raf_releaser(value: *const crate::Raf) {
    if value.is_null() {
        return;
    }
...
#[cfg(any(target_os = "android", target_os = "ios"))]
#[no_mangle]
pub extern "C" fn canvas_native_raf_start(raf: *const crate::Raf) {
    if raf.is_null() {
        return;
    }
    let raf = unsafe { &*raf };
...
#[cfg(any(target_os = "android", target_os = "ios"))]
#[no_mangle]
pub extern "C" fn canvas_native_raf_stop(raf: *const crate::Raf) {
    if raf.is_null() {
        return;
    }
    let raf = unsafe { &*raf };
...
pub extern "C" fn canvas_native_context_create_gl(
    ...
    let gl_context = gl_context as *const GLContext;
    let gl_context = unsafe { &*gl_context };

    gl_context.0.make_current();
    let gl_context = gl_context.0.clone();
...
fn canvas_native_webgl_create_no_window_internal(
    ...
    WebGLState::new_with_context(ctx, version)
}
...
pub extern "C" fn canvas_native_context_clip_str(
    ...
    if let Ok(rule) = FillRule::try_from(rule.to_string_lossy().as_ref()) {
        let context = unsafe { &mut *context };
        context.make_current();
        context.context.clip(Some(&mut path.0), Some(rule));
    }
}
...
pub extern "C" fn canvas_native_context_clip_rule_str(
    ...
    let rule = unsafe { CStr::from_ptr(rule) };
    if let Ok(rule) = FillRule::try_from(rule.to_string_lossy().as_ref()) {
        context.context.clip(None, Some(rule));
    }
}
...
pub extern "C" fn canvas_native_context_draw_image_dx_dy_context(
    ...
    let source = unsafe { &*source };
    source.make_current();
    ctx.draw_image_dx_dy(&image, dx, dy);
}
    ...
    #[cfg(any(target_os = "ios", target_os = "macos"))]
    {
        ...
        if let Some((image, origin)) = snapshot.backend_texture(false) {
            ...
            if let Some(image) = from_backend_texture(&mut ctx, &image, origin, info) {
                ctx.draw_image_dx_dy_dw_dh(&image, dx, dy, d_width, d_height);
            }
        }
    }
...
pub extern "C" fn canvas_native_context_fill_str(
    ...
    if let Ok(rule) = FillRule::try_from(rule.as_ref()) {
        let context = unsafe { &mut *context };
        context.make_current();
        context.context.fill_rule(None, rule);
    }
}
...
pub extern "C" fn canvas_native_context_fill_with_path_str(
    ...
    if let Ok(rule) = FillRule::try_from(rule.as_ref()) {
        let context = unsafe { &mut *context };
        context.make_current();
        context.context.fill_rule(Some(&mut path.0), rule);
    }
}
...
pub extern "C" fn canvas_native_context_is_point_in_path_str(
    ...
    FillRule::try_from(rule.as_ref()).map_or(false, |rule| {
        let context = unsafe { &*context };
        context.context.point_in_path(None, x, y, rule)
    })
}
...
pub extern "C" fn canvas_native_context_is_point_in_path_with_path_str(
    ...
    FillRule::try_from(rule.as_ref()).map_or(false, |rule| {
        ...
        context
            .context
            .point_in_path(Some(&path.0), x, y, rule)
    })
}
