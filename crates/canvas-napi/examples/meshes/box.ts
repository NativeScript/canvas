 ...
 */
export function createBoxMeshWithTangents(width: number, height: number, depth: number): Mesh {
	//    __________
	//   /         /|      y
	//  /   +y    / |      ^
	// /_________/  |      |
	...
	const vertices = new Float32Array(faces.length * verticesPerSide * f32sPerVertex);
	...
	for (let faceIndex = 0; faceIndex < faces.length; faceIndex++) {
		...
		for (let u = 0; u < 2; u++) {
			for (let v = 0; v < 2; v++) {
				for (let i = 0; i < 3; i++) {
					vertices[vertexOffset++] = normal[i] + (u == 0 ? -1 : 1) * tangent[i] + (v == 0 ? -1 : 1) * bitangent[i];
				}
				for (let i = 0; i < 3; i++) {
