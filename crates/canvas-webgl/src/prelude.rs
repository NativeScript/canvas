...
pub(crate) struct WebGLStateInner {
    ...
    // Cached GL limits and runtime safety probe
    ...
    gpu_safe: bool,
    // Per-program cached active uniform block counts
...
pub struct WebGLState {
    ...
    pub(crate) state: Rc<RefCell<WebGLStateInner>>,
    pub(crate) attributes: Rc<RefCell<ContextAttributes>>,
}
...
impl WebGLState {
    ...
    pub fn set_surface(
        ...
        self.context.set_surface(attr, width, height, handle)
    }
    ...
    pub fn new_with_context(
        ...
        let mut instance = Self {
            ...
            state: Rc::new(RefCell::new(WebGLStateInner {
                ...
                gpu_safe: false,
                program_active_uniform_blocks: HashMap::new(),
            })),
        };

        // Probe and cache device limits / runtime safety
        instance.probe_and_cache_limits();
    ...
    pub fn get_drawing_buffer_width(&self) -> i32 {
        self.context.get_surface_width()
    }

    pub fn get_drawing_buffer_height(&self) -> i32 {
        self.context.get_surface_height()
    }

    pub fn get_dimensions(&self) -> (i32, i32) {
        self.context.get_surface_dimensions()
    }

    pub(crate) fn set_antialias(&self, value: bool) {
        self.get_attributes_mut().set_antialias(value);
    }

    pub(crate) fn set_alpha(&self, value: bool) {
        self.get_attributes_mut().set_alpha(value);
    }
    pub(crate) fn set_webgl_version(&self, version: WebGLVersion) {
        self.get_state_mut().version = version;
    }

    pub(crate) fn set_gl_context(&mut self, context: canvas_core::gpu::gl::GLContext) {
        self.context = context;
    }
    pub fn get_webgl_version(&self) -> WebGLVersion {
        self.get_state().version
    }
    ...
    pub fn set_unpack_colorspace_conversion_webgl(&mut self, color_space: i32) {
        self.get_state_mut().unpack_colorspace_conversion_webgl = color_space;
    }

    pub fn set_flip_y(&mut self, flip: bool) {
        self.get_state_mut().flip_y = flip;
    }

    pub fn get_alpha(&self) -> bool {
        self.get_attributes().get_alpha()
    }

    pub fn get_antialias(&self) -> bool {
        self.get_attributes().get_antialias()
    }

    pub fn get_depth(&self) -> bool {
        self.get_attributes().get_depth()
    }

    pub fn get_fail_if_major_performance_caveat(&self) -> bool {
        self.get_attributes().get_fail_if_major_performance_caveat()
    }

    pub fn get_power_preference(&self) -> PowerPreference {
        self.get_attributes().get_power_preference()
    }

    pub fn get_premultiplied_alpha(&self) -> bool {
        self.get_attributes().get_premultiplied_alpha()
    }

    pub fn get_preserve_drawing_buffer(&self) -> bool {
        self.get_attributes().get_preserve_drawing_buffer()
    }

    pub fn get_stencil(&self) -> bool {
        self.get_attributes().get_stencil()
    }

    pub fn get_desynchronized(&self) -> bool {
        self.get_attributes().get_desynchronized()
    }

    pub fn get_xr_compatible(&self) -> bool {
        self.get_attributes().get_xr_compatible()
    }
    ...
    /// Probe GL for conservative capability values and a small runtime UBO test
    /// to detect broken/partial WebGL2/GLES3 drivers and cache results.
    pub fn probe_and_cache_limits(&mut self) {
        self.context.make_current();
        ...
        if self.get_version() == WebGLVersion::V2 {
            ...
            unsafe {
                gl_bindings::BindBuffer(gl_bindings::UNIFORM_BUFFER, 0);
                if buf != 0 {
                    gl_bindings::DeleteBuffers(1, &buf);
                }
            }
    ...
    /// Return the cached max combined texture units (0 if unknown)
    pub fn get_max_combined_texture_image_units(&self) -> i32 {
        self.state.borrow().max_combined_texture_image_units
    }

    /// Return the cached max uniform buffer bindings (0 if unknown)
    pub fn get_max_uniform_buffer_bindings(&self) -> i32 {
        self.state.borrow().max_uniform_buffer_bindings
    }

    /// Whether a conservative GPU path appears safe on this device
    pub fn is_gpu_safe(&self) -> bool {
        self.state.borrow().gpu_safe
    }
