...
const { GPUBufferUsage, GPUTextureUsage, ImageAsset } = require('../../canvas-napi.darwin-arm64.node');
...
export async function run(canvas) {
	...
	let planetTexture: GPUTexture;
	{
		...
		const saturn = import.meta.resolve('../assets/saturn.png');

		await imageBitmap.fromFile(saturn.replace('file://', ''));
	...
	let moonTexture: GPUTexture;
	{
		...
		const moon = import.meta.resolve('../assets/moon.png');

		await imageBitmap.fromFile(moon.replace('file://', ''));
