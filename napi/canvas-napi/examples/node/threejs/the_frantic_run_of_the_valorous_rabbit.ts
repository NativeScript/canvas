...
export function run(canvas, parent: HTMLElement) {
	parent.style.backgroundColor = '#dbe6e6';

	const TweenMax = require('../TweenMax.min');

	const containerLayout = document.createElement('view');
	...
	var fieldGameOver: HTMLElement, fieldDistance: HTMLElement, fieldTitle: HTMLElement;
  ...
  //*/
	...
	function updateDistance() {
		...
		var d = distance / 2;
		fieldDistance.textContent = `${Math.floor(d)}`;
	}
	...
	function initUI() {
		fieldTitle = document.createElement('text');
		fieldTitle.textContent = 'DISTANCE';
		fieldTitle.style.color = '#ffa873';
		fieldTitle.style.verticalAlign = 'top';
		fieldTitle.style.textAlign = 'center';
		fieldTitle.className = 'distanceTitle';

		fieldDistance = document.createElement('text');
		fieldDistance.style.verticalAlign = 'top';
		fieldDistance.style.textAlign = 'center';
		fieldDistance.textContent = '000';
		fieldDistance.style.color = '#dc5f45';
		fieldDistance.className = 'distanceValue';

		fieldGameOver = document.createElement('text');
		fieldGameOver.style.color = '#ffc5a2';
		fieldGameOver.textContent = 'Game Over';
		fieldGameOver.className = 'gameover';
		fieldGameOver.style.textAlign = 'center';

		const fieldInfo = document.createElement('view');

		//	fieldInfo.orientation = 'vertical';

		fieldInfo.appendChild(fieldTitle);
		fieldInfo.appendChild(fieldDistance);
		containerLayout.appendChild(fieldInfo);
		containerLayout.appendChild(fieldGameOver);

		console.log(containerLayout.parentElement === parent);
		if (containerLayout.parentElement !== parent) {
			parent.appendChild(containerLayout);
		}
	}
