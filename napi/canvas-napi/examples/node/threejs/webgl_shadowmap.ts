...
let camera, controls, scene, renderer, context;
//let container, stats;
...
function init(canvas) {
	SCREEN_WIDTH = canvas.clientWidth * window.devicePixelRatio;
	SCREEN_HEIGHT = canvas.clientHeight * window.devicePixelRatio;
...
function createScene() {
	...
	import('../fonts/helvetiker_bold.typeface.json').then((fontData) => {
	...
	*/
	...
	const base = import.meta.resolve('./models/gltf');

	const horse = import.meta.resolve('./models/gltf/Horse.glb');
	const flamingo = import.meta.resolve('./models/gltf/Flamingo.glb');
	const stork = import.meta.resolve('./models/gltf/Stork.glb');
	const parrot = import.meta.resolve('./models/gltf/Parrot.glb');

	fs.readFile(horse.replace('file://', '')).then((horse) => {
		gltfloader.parse(horse.buffer, '', function (gltf) {
	...
	fs.readFile(flamingo.replace('file://', '')).then((flamingo) => {
		gltfloader.parse(flamingo.buffer, '', function (gltf) {
			const mesh = gltf.scene.children[0];
	...
	fs.readFile(stork.replace('file://', '')).then((stork) => {
		gltfloader.parse(stork.buffer, '', function (gltf) {
			const mesh = gltf.scene.children[0];
	...
	fs.readFile(parrot.replace('file://', '')).then((parrot) => {
		gltfloader.parse(parrot.buffer, '', function (gltf) {
			const mesh = gltf.scene.children[0];
	...
	/*
		...
		gltfloader.load(horse, function(gltf) {
			const mesh = gltf.scene.children[0];
		...
		gltfloader.load(flamingo, function(gltf) {
			const mesh = gltf.scene.children[0];
			const clip = gltf.animations[0];
		...
		gltfloader.load(stork, function(gltf) {
			const mesh = gltf.scene.children[0];
			const clip = gltf.animations[0];
		...
		gltfloader.load(parrot, function(gltf) {
			const mesh = gltf.scene.children[0];
			const clip = gltf.animations[0];
...
export function webgl_shadowmap(canvas) {
	init(canvas);
	animate();
}
