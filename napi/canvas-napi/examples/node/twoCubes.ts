...
const { GPUBufferUsage, GPUCanvasContext, GPUDevice, GPUTextureUsage } = require('../../canvas-napi.darwin-arm64.node');
...
export async function run(canvas) {
	...
	const vertexPositionColorWGSLFile = import.meta.resolve('../shaders/vertexPositionColor.frag.wgsl');
	url = NSURL.alloc().initWithString(vertexPositionColorWGSLFile);

	data = NSData.dataWithContentsOfFile(url.path);
	const vertexPositionColorWGSL = NSString.alloc().initWithDataEncoding(data, NSUTF8StringEncoding).toString();
