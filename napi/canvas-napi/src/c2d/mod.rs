...
use napi::bindgen_prelude::{
    ClassInstance, Either, Either3, Either4, FromNapiValue, ObjectFinalize,
};
use napi::*;
...
impl CanvasRenderingContext2D {
  ...
  pub fn with_mtl_view_device_queue(
    ...
    CanvasRenderingContext2D {
      context: Box::into_raw(Box::new(ctx_2d) as _),
    }
  }
    ...
    match (path, rule) {
      ...
      (Some(path), Some(rule)) => {
        if let Some(rule) = rule.into_utf8().ok() {
          if let Ok(rule) = rule.as_str() {
            match rule {
              "nonzero" => canvas_c::canvas_native_context_clip(
                self.context,
                path.path,
                CanvasFillRule::NonZero,
              ),
              "evenodd" => canvas_c::canvas_native_context_clip(
                self.context,
                path.path,
  ...
  pub fn create_conic_gradient(
    ...
    CanvasGradient { style: gradient }.into_instance(env)
  }
  ...
  pub fn create_image_data(
    &self,
    env: Env,
    width_or_image_data: Either<f64, ClassInstance<ImageData>>,
    height: Option<f64>,
    ...
    match width_or_image_data {
      Either::A(width) => {
        if let Some(height) = height {
          return ImageData {
            data: unsafe {
              Arc::from_raw(canvas_c::canvas_native_context_create_image_data(
                width as i32,
                height as i32,
              ))
            },
  ...
  pub fn create_pattern(
    &self,
    env: Env,
    image: ClassInstance<ImageAsset>,
    repetition: Option<JsString>,
    ...
    let pattern = canvas_c::canvas_native_context_create_pattern_asset(
      self.context,
      image.asset.as_ref(),
      repetition,
    );
    CanvasPattern { style: pattern }.into_instance(env)
  }
  ...
  pub fn draw_focus_if_needed(
    &self,
    element_path: Either<JsObject, ClassInstance<Path2D>>,
    element: Option<JsObject>,
  ) {
  }
  ...
  pub fn draw_image(
    &self,
    image: Either3<ClassInstance<ImageAsset>, HTMLImageSource, HTMLCanvasSource>,
    sx: Option<f64>,
    sy: Option<f64>,
    s_width: Option<f64>,
    s_height: Option<f64>,
    ...
    match (sx, sy, s_width, s_height, dx, dy, d_height, d_width) {
      (Some(dx), Some(dy), None, None, None, None, None, None) => match image {
        Either3::A(image) => {
        ...
        Either3::B(source) => {
          let image = Arc::as_ptr(&source.image.asset);
          canvas_c::canvas_native_context_draw_image_dx_dy_asset(
            self.context,
            image as _,
            dx as f32,
        ...
        Either3::C(source) => match source.context {
          Either4::A(c2d) => canvas_c::canvas_native_context_draw_image_dx_dy_context(
            self.context,
            c2d.context,
            dx as f32,
            dy as f32,
          ),
          Either4::B(gl) => canvas_c::canvas_native_context_draw_image_dx_dy_webgl(
            self.context,
            gl.state,
            dx as f32,
            dy as f32,
          ),
          Either4::C(gl2) => canvas_c::canvas_native_context_draw_image_dx_dy_webgl(
            self.context,
            gl2.state,
            dx as f32,
            dy as f32,
      ...
      (Some(dx), Some(dy), Some(d_width), Some(d_height), None, None, None, None) => match image {
        Either3::A(image) => {
          let image = Arc::as_ptr(&image.asset);
        ...
        Either3::C(source) => match source.context {
          Either4::A(c2d) => canvas_c::canvas_native_context_draw_image_dx_dy_dw_dh_context(
            self.context,
            c2d.context,
            dx as f32,
            dy as f32,
          ...
          Either4::B(gl) => canvas_c::canvas_native_context_draw_image_dx_dy_dw_dh_webgl(
            self.context,
            gl.state,
            dx as f32,
            dy as f32,
          ...
          Either4::C(gl2) => canvas_c::canvas_native_context_draw_image_dx_dy_dw_dh_webgl(
            self.context,
            gl2.state,
            dx as f32,
            dy as f32,
      ...
      (
        ...
        Either3::C(source) => match source.context {
          Either4::A(c2d) => canvas_c::canvas_native_context_draw_image_context(
            self.context,
            c2d.context,
            sx as f32,
            sy as f32,
          ...
          Either4::B(gl) => canvas_c::canvas_native_context_draw_image_webgl(
            self.context,
            gl.state,
            sx as f32,
            sy as f32,
          ...
          Either4::C(gl2) => canvas_c::canvas_native_context_draw_image_webgl(
            self.context,
            gl2.state,
            sx as f32,
            sy as f32,
  ...
  pub fn fill(&self, path: Option<ClassInstance<Path2D>>, rule: Option<JsString>) {
    match (path, rule) {
      (None, None) => canvas_c::canvas_native_context_fill(self.context, CanvasFillRule::NonZero),
      (Some(path), None) => canvas_c::canvas_native_context_fill_with_path(
        self.context,
        path.path,
        CanvasFillRule::NonZero,
      ),
      ...
      (Some(path), Some(rule)) => {
        if let Some(rule) = rule.into_utf8().ok() {
          if let Ok(rule) = rule.as_str() {
            match rule {
              "nonzero" => canvas_c::canvas_native_context_fill_with_path(
                self.context,
                path.path,
                CanvasFillRule::NonZero,
              ),
              "evenodd" => canvas_c::canvas_native_context_fill_with_path(
                self.context,
                path.path,
                CanvasFillRule::EvenOdd,
              ),
  ...
  pub fn get_image_data(
    ...
    ImageData {
      data: unsafe {
        Arc::from_raw(canvas_c::canvas_native_context_get_image_data(
          self.context,
          x as f32,
          y as f32,
          width as f32,
  ...
  pub fn is_point_in_path(
    &self,
    x_or_path: Either<f64, ClassInstance<Path2D>>,
    y: Option<f64>,
    rule_or_y: Option<Either<f64, JsString>>,
    rule: Option<JsString>,
  ) -> bool {
    ...
    match x_or_path {
      ...
      Either::B(path) => match (y, rule_or_y, rule) {
        (Some(x), Some(rule_or_y), Some(rule)) => match rule_or_y {
          Either::A(y) => {
            if let Some(rule) = rule.into_utf8().ok() {
              if let Ok(rule) = rule.as_str() {
                match rule {
        ...
        (Some(x), Some(rule_or_y), _) => match rule_or_y {
          Either::A(y) => canvas_c::canvas_native_context_is_point_in_path(
            self.context,
            x as f32,
            y as f32,
            rule_inner,
  ...
  pub fn is_point_in_stroke(
    ...
    match x_or_path {
      ...
      Either::B(path) => match (x_or_y, y) {
        (Some(x), Some(y)) => canvas_c::canvas_native_context_is_point_in_stroke_with_path(
          self.context,
          path.path,
          x as f32,
          y as f32,
  ...
  pub fn put_image_data(
    &self,
    image_data: ClassInstance<ImageData>,
    dx: f64,
    dy: f64,
    dirty_x: Option<f64>,
    dirty_y: Option<f64>,
  ...
  pub fn set_transform(
    &self,
    a: Either<f64, ClassInstance<DOMMatrix>>,
    b: Option<f64>,
    c: Option<f64>,
    d: Option<f64>,
    e: Option<f64>,
