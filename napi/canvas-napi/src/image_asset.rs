...
impl Task for AsyncFileTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    match CString::new(self.path.as_str()) {
      Ok(path) => {
        let done = canvas_native_image_asset_load_from_path(Arc::as_ptr(&self.asset), path.as_ptr());
        Ok(done)
      }
      Err(e) => Err(Error::from_reason(e.to_string()))?,
    }
...
impl ImageAsset {
  ...
  #[napi(js_name = "fromFileSync")]
  pub fn load_file_sync(&self, path: String) -> bool {
  ...
  #[napi(js_name = "fromFile", ts_return_type = "Promise<boolean>")]
  pub fn load_file(&self, path: String) -> AsyncTask<AsyncFileTask> {
