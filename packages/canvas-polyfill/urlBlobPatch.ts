...
// Only patch URL if present and avoid overwriting existing helpers
if (typeof URL !== 'undefined') {
	...
	if (typeof (URL as any).createObjectURL === 'undefined') {
		(URL as any).createObjectURL = function (object, options = null) {
			try {
	...
	if (typeof (URL as any).revokeObjectURL === 'undefined') {
		(URL as any).revokeObjectURL = function (url) {
			BLOB_STORE.delete(url);
		};
	}

	// Attach InternalAccessor without clobbering an existing implementation
	const existingInternal = (URL as any).InternalAccessor || {};
	if (typeof existingInternal.getData !== 'function') {
		existingInternal.getData = function (url) {
			return BLOB_STORE.get(url);
		};
	}
	(URL as any).InternalAccessor = existingInternal;

	if (__ANDROID__) {
		if (typeof (URL as any).InternalAccessor.getPath !== 'function') {
			(URL as any).InternalAccessor.getPath = (url: string) => {
				const blob = (URL as any).InternalAccessor.getData(url);
				if (!blob) {
					return '';
				}
				if (blob.path) {
