...
public:
    ...
    std::shared_ptr<ConcurrentMap<int, std::shared_ptr<OnImageAssetLoadCallbackHolder>>> OnImageAssetLoadCallbackHolder_ = std::make_shared<ConcurrentMap<int, std::shared_ptr<OnImageAssetLoadCallbackHolder>>>();

    std::shared_ptr<ConcurrentMap<int, std::shared_ptr<OnImageBitmapLoadCallbackHolder>>> OnImageBitmapLoadCallbackHolder_ = std::make_shared<ConcurrentMap<int, std::shared_ptr<OnImageBitmapLoadCallbackHolder>>>();
    ...
    int GetAssetReader(){
        return this->assetFD_[0];
    }

    int GetAssetWriter(){
        return this->assetFD_[1];
    }

    int taskId_;

    static int HandleAssetLoopCallback(int fd, int events, void *data);
    ...
    struct Task {
        ...
        bool done_;
    public:
        Task(int type, int id, bool done) : type_(type), id_(id), done_(done) {}
