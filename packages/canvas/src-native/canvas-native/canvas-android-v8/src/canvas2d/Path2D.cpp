    ...
    if (!args.IsConstructCall()) {
        ...
        if (args.Length() > 0) {
            ...
            if (obj->IsString()) {
                ...
                Path2D *path = new Path2D(std::move(
                        canvas_native_path_create_with_string(d)));
                ...
                if (name->ToString(context).ToLocalChecked()->StringEquals(
                        Helpers::ConvertToV8String(isolate, "Path2D"))) {
                    auto path_from = GetPointer(obj->ToObject(context).ToLocalChecked());
...
void Path2D::AddPath(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (args[0]->IsObject()) {
        ...
        auto cache = Caches::Get(isolate);
        auto ctor = cache->Path2DCtor->Get(isolate);
...
void Path2D::ToSVG(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    args.GetReturnValue().Set(
            Helpers::ConvertToV8String(isolate, std::string(svg.begin(), svg.end()))
    );
}
