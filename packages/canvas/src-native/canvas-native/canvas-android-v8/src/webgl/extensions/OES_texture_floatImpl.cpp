...
v8::Local <v8::Function> OES_texture_floatImpl::GetCtor(v8::Isolate *isolate) {
    auto cache = Caches::Get(isolate);
    auto ctor = cache->OES_texture_floatImplCtor.get();
    if (ctor != nullptr) {
    ...
    cache->OES_texture_floatImplCtor = std::make_unique<v8::Persistent<v8::Function>>(isolate, func);
    return func;
}
