...
v8::Local<v8::Object> WebGLVertexArrayObject::NewInstance(v8::Isolate *isolate, uint32_t object) {
    v8::Locker locker(isolate);
    v8::Isolate::Scope isolate_scope(isolate);
    v8::EscapableHandleScope handle_scope(isolate);
    auto ctorFunc = GetCtor(isolate);
    auto result = ctorFunc->InstanceTemplate()->NewInstance(isolate->GetCurrentContext()).ToLocalChecked();
    Helpers::SetPrivate(isolate, result, "instance", v8::Uint32::New(isolate, object));
    Helpers::SetInstanceType(isolate, result, ObjectType::WebGLVertexArrayObject);
