    ...
    if let Some((bytes, info)) = bytes {
        unsafe {
           ...
           let xform_len =  env.get_array_length(&xform).unwrap_or_default();
            ...
            if xform_len == 0 || tex_len == 0 { return; }

            let mut xform_buf: Vec<f32> = Vec::with_capacity( xform_len as usize);
            let mut tex_buf: Vec<f32> = Vec::with_capacity( tex_len as usize);


            if let (Ok(_), Ok(_)) = (env.get_float_array_region(xform, 0, xform_buf.as_mut_slice()), env.get_float_array_region(tex, 0, tex_buf.as_mut_slice())){
                let context = context as *mut CanvasRenderingContext2D;

                let context = unsafe { &mut *context };

                if let Some(image) = from_image_slice(bytes.as_slice(), info.width() as i32, info.height() as i32) {
                    context.make_current();
                    let mut context = context.get_context_mut();
                    ...
                    let colors_value: Option<Vec<Color>> = if colors_len > 0 {
                        ...
                        Some(colors_buf.into_iter().map(|color| Color::new(color as u32)).collect())
                    }else { None };
                    context.draw_atlas(
                        &image, xform_buf.as_slice(), tex_buf.as_slice(), colors_value.as_deref(), CompositeOperationType::from(blend_mode)
                    )
                }
            }
        }
