	...
	textureRotate(canvas) {
		...
		function init() {
			...
			for (let i = 0; i < 16; i++) {
				...
				const dude = new PIXI.Sprite(
					textures[i < 8 ? i * 2 : (i - 8) * 2 + 1]
				);
				dude.scale.x = 0.5;
				dude.scale.y = 0.5;
				// show it in grid
...
void main()
{
    ...
    gl_FragColor = outColor;
		...
		const gridShader: any = PIXI.Shader.from(
			vertexSrc,
			fragmentGridSrc,
			gridUniforms
		);
...
void main()
{
    ...
    gl_FragColor = color;
		...
		const rippleShader: any = PIXI.Shader.from(
			vertexSrc,
			fragmentRippleSrc,
			rippleUniforms
		);
...
void main()
{
    ...
    gl_FragColor = vec4(color);
		...
		const noiseShader: any = PIXI.Shader.from(
			vertexSrc,
			fragmentNoiseSrc,
			noiseUniforms
		);
...
void main()
{
    ...
    gl_FragColor = vec4(outColor,1.0);
		...
		const waveShader: any = PIXI.Shader.from(
			vertexSrc,
			fragmentWaveSrc,
			waveUniforms
		);
...
void main()
{
    ...
    gl_FragColor = mix(ripple, wave,noise.r);
		...
		const combineShader: any = PIXI.Shader.from(
			vertexSrc,
			fragmentCombineSrc,
			combineUniforms
		);
...
`
		...
		const triangle = new PIXI.Mesh(
			...
			new (PIXI as any).Shader(program, {
				uSamplerTexture: PIXI.Texture.from(
					this.root + "/images/bg_scene_rotate.jpg"
				),
			})
		);
		...
		const triangle3 = new PIXI.Mesh(
			...
			new (PIXI as any).Shader(program, {
				uSamplerTexture: PIXI.Texture.from(
					this.root + "/images/bg_displacement.jpg"
				),
			})
		);
...
// create an array of image ids..
		const fruits = [
			this.root + '/images/rt_object_01.png',
			this.root + '/images/rt_object_02.png',
			this.root + '/images/rt_object_03.png',
			this.root + '/images/rt_object_04.png',
			this.root + '/images/rt_object_05.png',
			this.root + '/images/rt_object_06.png',
			this.root + '/images/rt_object_07.png',
			this.root + '/images/rt_object_08.png',
		];
...
// used for spinning!
	...
	meshBasic(canvas) {
		...
		const strip = new PIXI.SimpleRope(
			PIXI.Texture.from(this.root + "/images/snake.png"),
			points
		);
	...
	bitmapFont(canvas) {
		...
		app.loader
			.add("desyrel", this.root + "/bitmap-font/desyrel.xml")
			.load(onAssetsLoaded);

		function onAssetsLoaded() {
			const bitmapFontText = new (PIXI as any).BitmapText(
	...
	explosion(canvas) {
		...
		app.loader
			.add("spritesheet", this.root + "/spritesheet/mc.json")
			.load(onAssetsLoaded);
	}
...
// Listen for animate update
	...
	cacheAsBitmap(canvas) {
		...
		// load resources
		app.loader
			.add("spritesheet", this.root + "/spritesheet/monsters.json")
			.load(onAssetsLoaded);
...
// create a bounding box for the little dudes
	...
	animatedJet(canvas) {
		...
		app.loader
			.add(this.root + '/spritesheet/fighter.json')
			.load(onAssetsLoaded);
...
// // BEZIER CURVE 2 ////
		...
		bezier2.lineTextureStyle({
			width: 10, texture: sprite.texture
		});
