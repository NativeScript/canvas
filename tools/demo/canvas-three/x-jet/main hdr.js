...
function init() {
  ...
  renderer.setPixelRatio(window.devicePixelRatio / 2.5) // reduce this value for better quality 1 is original too heavy and not needed for mobile
  ...
  const hdrCubeMap = new HDRCubeTextureLoader().setPath('./hdr512/').load(hdrUrls, function () {
  ...
  loader.load('TriniShip.glb', function (gltf) {
    ship = gltf.scene
    ...
    loader.load('drone.glb', function (gltf) {
      ...
      const map = new THREE.TextureLoader().load('./explosion.png')
...
function simpleCollisionCheck(shot) {
  ...
  drones.forEach((drone) => {
    ...
    if (
      Math.abs(dvec.x - shot.x) < 100 &&
      Math.abs(dvec.y - shot.y) < 100 &&
      Math.abs(dvec.z - shot.z) < 100
    )
      drone.userData.mustExplode = true
  })
}
...
function simulateKeyPress(delta, target) {
  ...
  switch (data.currentKey) {
    case 'left':
      rotationSpeedX = THREE.MathUtils.lerp(
        rotationSpeedX,
        -1,
        (data.MAX_PRESS_TIME - data.timeLeft) / droneRotationEasing
      )
      break
    case 'right':
      rotationSpeedX = THREE.MathUtils.lerp(
      ...
      break
    case 'up':
      rotationSpeedY = THREE.MathUtils.lerp(
        rotationSpeedY,
        1,
        (data.MAX_PRESS_TIME - data.timeLeft) / droneRotationEasing
      )
  ...
  if (target.position.x > -500 && target.position.y < 500)
    target.position.x += rotationSpeedX * data.droneMovementSpeed * delta
  if (target.position.y > 0 && target.position.y < 500)
    target.position.y += rotationSpeedY * data.droneMovementSpeed * delta
