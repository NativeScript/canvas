	...
	const API = {
		...
		noise3D(x, y, z) {
			...
			var c = (() => {
				...
				return lookup3D.get((yins - zins + 1) | ((xins - yins + 1) << 1) | ((xins - zins + 1) << 2) | (inSum << 3) | ((inSum + zins) << 5) | ((inSum + yins) << 7) | ((inSum + xins) << 9));
			})();
			var i,
				value = 0;
			while (c !== undefined) {
		...
		noise4D(x, y, z, w) {
			...
			var c = (() => {
				...
				return lookup4D.get((zins - wins + 1) | ((yins - zins + 1) << 1) | ((yins - wins + 1) << 2) | ((xins - yins + 1) << 3) | ((xins - zins + 1) << 4) | ((xins - wins + 1) << 5) | (inSum << 6) | ((inSum + wins) << 8) | ((inSum + zins) << 11) | ((inSum + yins) << 14) | ((inSum + xins) << 17));
			})();
			var i,
				value = 0;
			while (c !== undefined) {
