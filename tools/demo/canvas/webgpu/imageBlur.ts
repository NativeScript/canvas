import { Canvas, GPUBufferUsage, GPUDevice, GPUTextureUsage, ImageAsset } from '@nativescript/canvas';
...
export async function run(canvas: Canvas) {
	...
	function frame() {
		...
		computePass.dispatchWorkgroups(Math.ceil(srcWidth / blockDim), Math.ceil(srcHeight / batch[1]));

		computePass.setBindGroup(1, computeBindGroup1);
		computePass.dispatchWorkgroups(Math.ceil(srcHeight / blockDim), Math.ceil(srcWidth / batch[1]));
