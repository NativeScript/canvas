import { vec3, vec2 } from '../../../wgpu-matrix/wgpu-matrix';
import { GPUBuffer, GPUBindGroup, GPUBufferUsage, GPUDevice } from '@nativescript/canvas';
 ...
 */
export const createMeshRenderable = (device: GPUDevice, mesh: Mesh, storeVertices = false, storeIndices = false): Renderable => {
	// Define buffer usage
	const vertexBufferUsage = storeVertices ? GPUBufferUsage.VERTEX | GPUBufferUsage.STORAGE : GPUBufferUsage.VERTEX;
	const indexBufferUsage = storeIndices ? GPUBufferUsage.INDEX | GPUBufferUsage.STORAGE : GPUBufferUsage.INDEX;
...
export const getMeshPosAtIndex = (mesh: Mesh, index: number) => {
	const arr = new Float32Array(mesh.vertices.buffer, index * mesh.vertexStride + 0, 3);
	return vec3.fromValues(arr[0], arr[1], arr[2]);
};

export const getMeshNormalAtIndex = (mesh: Mesh, index: number) => {
	const arr = new Float32Array(mesh.vertices.buffer, index * mesh.vertexStride + 3 * Float32Array.BYTES_PER_ELEMENT, 3);
	return vec3.fromValues(arr[0], arr[1], arr[2]);
};

export const getMeshUVAtIndex = (mesh: Mesh, index: number) => {
	const arr = new Float32Array(mesh.vertices.buffer, index * mesh.vertexStride + 6 * Float32Array.BYTES_PER_ELEMENT, 2);
