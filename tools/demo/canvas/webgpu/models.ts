...
export function convertMeshToTypedArrays(mesh: Mesh, scale: number, offset = [0, 0, 0]) {
	const { positions, normals, triangles } = mesh;
	const scaledPositions = positions.map((p) => p.map((v, i) => v * scale + offset[i % 3]));
...
function createSphereTypedArrays(radius: number, widthSegments = 32, heightSegments = 16, randomness = 0) {
	const { vertices: verticesWithUVs, indices } = createSphereMesh(radius, widthSegments, heightSegments, randomness);
...
function flattenNormals({ vertices, indices }: { vertices: Float32Array; indices: Uint32Array }) {
	...
	for (let i = 0; i < indices.length; i += 3) {
		...
		const normal = vec3.normalize(vec3.cross(vec3.normalize(vec3.subtract(positions[1], positions[0])), vec3.normalize(vec3.subtract(positions[2], positions[1]))));
