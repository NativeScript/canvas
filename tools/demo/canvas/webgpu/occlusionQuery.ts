...
import { Canvas, GPUCanvasContext, GPUDevice } from '@nativescript/canvas';
import { mat4 } from '../../wgpu-matrix/wgpu-matrix';
...
type TypedArrayView = Int8Array | Uint8Array | Int16Array | Uint16Array | Int32Array | Uint32Array | Float32Array | Float64Array;
...
{ position: [ 0,  0,  1], id: "🟪", color: [0.5, 0, 0.5, 1] },
	...
	function createBufferWithData(dev: GPUDevice, data: TypedArrayView, usage: GPUBufferUsageFlags, label: string) {
...
20, 21, 22, 20, 22, 23, // -z face
	...
	const renderPassDescriptor: GPURenderPassDescriptor = {
		...
		occlusionQuerySet: querySet as never,
	};

	const lerp = (a: number, b: number, t: number) => a + (b - a) * t;
	const lerpV = (a: number[], b: number[], t: number) => a.map((v, i) => lerp(v, b[i], t));
	...
	function render(now: number) {
		...
		const projection = mat4.perspective((30 * Math.PI) / 180, (canvas.width as never) / (canvas.height as never), 0.5, 100);
