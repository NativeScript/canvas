...
import { Canvas, GPUCanvasContext, GPUDevice, GPUTexture } from '@nativescript/canvas';
...
export async function run(canvas: Canvas) {
	...
	{
		const imageBitmap = new global.ImageAsset();

		await imageBitmap.fromFile('~/assets/file-assets/webgpu/saturn.png');

		global['saturn'] = imageBitmap;
	...
	{
		const imageBitmap = new global.ImageAsset();
		await imageBitmap.fromFile('~/assets/file-assets/webgpu/moon.png');
		global['moon'] = imageBitmap;
	...
	function createSphereRenderable(radius: number, widthSegments = 32, heightSegments = 16, randomness = 0) {
	...
	const asteroids = [createSphereRenderable(0.01, 8, 6, 0.15), createSphereRenderable(0.013, 8, 6, 0.15), createSphereRenderable(0.017, 8, 6, 0.15), createSphereRenderable(0.02, 8, 6, 0.15), createSphereRenderable(0.03, 16, 8, 0.15)];
