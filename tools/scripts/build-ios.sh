...
export RUSTFLAGS="$CARGO_FLAGS -Zlocation-detail=none -Zunstable-options -Cpanic=immediate-abort"
...
if [[ $IS_RELEASE == true ]]; then
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build -Z build-std='std,panic_abort' --target aarch64-apple-ios $BUILD_FLAG $FEATURE_FLAGS -p canvas-ios
  cp "$IOS_ARM_64_PHONE_OUTPUT_RELEASE_DIR" "$IOS_LIB_ARM_64_PHONE/$OUTPUT_LIB_NAME"
else
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build -Z build-std='std,panic_abort' --target aarch64-apple-ios $FEATURE_FLAGS -p canvas-ios
...
if [[ $IS_RELEASE == true ]]; then
  cd "$NATIVE_SRC"
  RUST_BACKTRACE=1 cargo +nightly build -Z build-std='std,panic_abort' --target x86_64-apple-ios $BUILD_FLAG $FEATURE_FLAGS -p canvas-ios
 cp "$IOS_X86_64_SIM_OUTPUT_RELEASE_DIR" "$IOS_LIB_X86_64_SIM/$OUTPUT_LIB_NAME"
else
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build -Z build-std='std,panic_abort' --target x86_64-apple-ios $FEATURE_FLAGS -p canvas-ios
...
if [[ $IS_RELEASE == true ]]; then
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build -Z build-std='std,panic_abort' --target aarch64-apple-ios-sim $BUILD_FLAG $FEATURE_FLAGS -p canvas-ios
 cp "$IOS_ARM_64_SIM_OUTPUT_RELEASE_DIR" "$IOS_LIB_ARM_64_SIM/$OUTPUT_LIB_NAME"
else
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build -Z build-std='std,panic_abort' --target aarch64-apple-ios-sim $FEATURE_FLAGS -p canvas-ios
